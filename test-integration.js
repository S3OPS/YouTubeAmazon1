...
async function runTests() {
    ...
    if (await test('GET /api/health returns status ok', async () => {
    ...
    if (await test('GET /api/health shows configured status', async () => {
        const response = await fetch('http://localhost:3000/api/health');
        const data = await response.json();
        if (typeof data.configured !== 'boolean') throw new Error('Configured status missing');
    })) passed++; else failed++;

    // Test 3: Products endpoint responds (may return error in demo mode)
    if (await test('GET /api/products responds', async () => {
    ...
    if (await test('GET /api/products returns JSON', async () => {
        const response = await fetch('http://localhost:3000/api/products');
        const data = await response.json();
    ...
    if (await test('GET / serves HTML page', async () => {
        const response = await fetch('http://localhost:3000/');
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const html = await response.text();
        if (!html.includes('<html')) throw new Error('Not an HTML page');
    })) passed++; else failed++;

    // Test 6: Static files are served
    if (await test('GET /app.js serves JavaScript', async () => {
